...
#include <Memory/WrappedResource.h>
    ...
    , m_PipelineState(nullptr)
{
    ...
    m_UploadBuffer = MakeUnique<UploadBuffer>(device);

    m_ResourceStateTracker = MakeUnique<ResourceStateTracker>();

    for (int i = 0; i < D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES; ++i)
    {
        m_DynamicDescriptorHeap[i] =
            MakeUnique<DynamicDescriptorHeap>(device, static_cast<D3D12_DESCRIPTOR_HEAP_TYPE>(i));
...
ComPtr<ID3D12Resource> CommandList::CopyBuffer(size_t bufferSize, const void* bufferData, D3D12_RESOURCE_FLAGS flags)
{
    ...
    else
    {
        ...
        ThrowIfFailed(d3d12Device->CreateCommittedResource(
            ADDR(CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT)), D3D12_HEAP_FLAG_NONE,
            ADDR(CD3DX12_RESOURCE_DESC::Buffer(bufferSize, flags)), D3D12_RESOURCE_STATE_COMMON, nullptr,
        ...
        if (bufferData != nullptr)
        {
            ...
            ThrowIfFailed(d3d12Device->CreateCommittedResource(
                ADDR(CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD)), D3D12_HEAP_FLAG_NONE,
                ADDR(CD3DX12_RESOURCE_DESC::Buffer(bufferSize)), D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
...
void CommandList::GenerateMips_UAV(const std::shared_ptr<Texture>& texture, bool isSRGB)
{
    ...
    {
        m_GenerateMipsPSO = MakeUnique<GenerateMipsPSO>(m_Device);
    }
    ...
    const std::shared_ptr<Texture>& panoTexture)
{
    ...
    {
        m_PanoToCubemapPSO = MakeUnique<PanoToCubemapPSO>(m_Device);
    }
    ...
    if ((cubemapDesc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS) == 0)
    {
        ...
        ThrowIfFailed(d3d12Device->CreateCommittedResource(
            ADDR(CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT)), D3D12_HEAP_FLAG_NONE, &stagingDesc,
    ...
    for (u32 mipSlice = 0; mipSlice < cubemapDesc.MipLevels; )
    {
        ...
        u32 numMips = std::min<u32>(5, cubemapDesc.MipLevels - mipSlice);

        panoToCubemapCB.FirstMip = mipSlice;
        panoToCubemapCB.CubemapSize =
            std::max<u32>(static_cast<u32>(cubemapDesc.Width), cubemapDesc.Height) >> mipSlice;
...
RefPtr<Scene> CommandList::CreateScene(const VertexCollection& vertices, const IndexCollection& indices)
{
    if (vertices.empty())
    {
        return nullptr;
    ...
    auto material = MakeRef<Material>(Material::White);

    mesh->CreatePrimitive(vertexBuffer, indexBuffer);
...
std::shared_ptr<Scene> CommandList::CreateSphere(float radius, u32 tessellation, bool reversWinding)
{
...
std::shared_ptr<Scene> CommandList::CreateCylinder(float radius, float height, u32 tessellation,
    bool reverseWinding)
{
    if (tessellation < 3)
        throw std::out_of_range("tessellation parameter out of range");
...
std::shared_ptr<Scene> CommandList::CreateCone(float radius, float height, u32 tessellation, bool reverseWinding)
{
    if (tessellation < 3)
        throw std::out_of_range("tessellation parameter out of range");
...
std::shared_ptr<Scene> CommandList::CreateTorus(float radius, float thickness, u32 tessellation,
    bool reverseWinding)
{
...
void CommandList::ClearUnorderedAccessViewUint(const std::shared_ptr<UnorderedAccessView>& view, const u32 value[4])
{
    assert(view);

    auto gpuHandle = m_DynamicDescriptorHeap[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV]->CopyDescriptor(*this, view->GetDescriptorHandle());
    m_d3d12CommandList->ClearUnorderedAccessViewUint(gpuHandle, view->GetDescriptorHandle(), view->GetResource()->GetD3D12Resource().Get(), value, 0, nullptr);
   // TransitionBarrier(texture, D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, true);
}

void CommandList::ClearUnorderedAccessViewFloat(const std::shared_ptr<UnorderedAccessView>& view, const float value[4])
{
    view;
    value;
}
...
void CommandList::CopyTextureSubresource(const std::shared_ptr<Texture>& texture, u32 firstSubresource,
    u32 numSubresources, D3D12_SUBRESOURCE_DATA* subresourceData)
{
    ...
    if (destinationResource)
    {
        ...
        ThrowIfFailed(d3d12Device->CreateCommittedResource(
            ADDR(CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD)), D3D12_HEAP_FLAG_NONE,
            ADDR(CD3DX12_RESOURCE_DESC::Buffer(requiredSize)), D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
...
void CommandList::SetGraphicsDynamicConstantBuffer(u32 rootParameterIndex, size_t sizeInBytes,
    const void* bufferData)
{
...
void CommandList::SetVertexBuffers(u32                                          startSlot,
...
void CommandList::SetVertexBuffer(u32 slot, const std::shared_ptr<VertexBuffer>& vertexBuffer)
{
    SetVertexBuffers(slot, { vertexBuffer });
}

void CommandList::SetDynamicVertexBuffer(u32 slot, size_t numVertices, size_t vertexSize,
    const void* vertexBufferData)
{
...
void CommandList::SetIndexBuffer(const std::shared_ptr<IndexBuffer>& indexBuffer)
{
    if (indexBuffer)
    {
        ...
        m_d3d12CommandList->IASetIndexBuffer(ADDR(indexBuffer->GetIndexBufferView()));
    }
}
...
void CommandList::SetGraphicsDynamicStructuredBuffer(u32 slot, size_t numElements, size_t elementSize,
    const void* bufferData)
{
...
void CommandList::SetComputeDynamicStructuredBuffer(u32 slot, size_t numElements, size_t elementSize,
    const void* bufferData)
{
    size_t bufferSize = numElements * elementSize;
...
void CommandList::SetConstantBufferView(u32 rootParameterIndex, const std::shared_ptr<ConstantBuffer>& buffer,
...
void CommandList::SetShaderResourceView(u32 rootParameterIndex, const std::shared_ptr<Buffer>& buffer,
    D3D12_RESOURCE_STATES stateAfter, size_t bufferOffset)
{
    if (buffer)
    {
...
void CommandList::SetUnorderedAccessView(u32 rootParameterIndex, const std::shared_ptr<Buffer>& buffer,
    D3D12_RESOURCE_STATES stateAfter, size_t bufferOffset)
{
    if (buffer)
    {
...
void CommandList::SetShaderResourceView(u32 rootParameterIndex, u32 descriptorOffset,
...
void CommandList::SetShaderResourceView(int32_t rootParameterIndex, u32 descriptorOffset,
...
void CommandList::SetUnorderedAccessView(u32 rootParameterIndex, u32 descriptorOffset,
...
void CommandList::SetConstantBufferView(u32 rootParameterIndex, u32 descriptorOffset,
...
void CommandList::DrawIndexed(u32 indexCount, u32 instanceCount, u32 startIndex, int32_t baseVertex,
    u32 startInstance)
{
    FlushResourceBarriers();
