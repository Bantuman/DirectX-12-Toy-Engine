...
#include <Asset\MaterialAssetHandler.h>

ActiveWindowProperties Application::WindowProperties;

UniquePtr<TextureAssetHandler> TextureAssetHandler::m_sTextureAssetHandlerSingleton;
UniquePtr<MeshAssetHandler> MeshAssetHandler::m_sMeshAssetHandlerSingleton;
UniquePtr<MaterialAssetHandler> MaterialAssetHandler::m_sMaterialAssetHandlerSingleton;
...
void Application::Create(HINSTANCE hInst)
{
    if (!gs_pSingelton)
    {
        ...
        TextureAssetHandler::m_sTextureAssetHandlerSingleton = MakeUnique<TextureAssetHandler>();
        TextureAssetHandler::m_sTextureAssetHandlerSingleton->Initialize();
        spdlog::info("TextureAssetHandler class created.");

        MaterialAssetHandler::m_sMaterialAssetHandlerSingleton = MakeUnique<MaterialAssetHandler>();
        MaterialAssetHandler::m_sMaterialAssetHandlerSingleton->Initialize();
        spdlog::info("MaterialAssetHandler class created.");

        MeshAssetHandler::m_sMeshAssetHandlerSingleton = MakeUnique<MeshAssetHandler>();
        MeshAssetHandler::m_sMeshAssetHandlerSingleton->Initialize();
        spdlog::info("MeshAssetHandler class created.");
    }
}
...
RefPtr<Window> Application::CreateRenderWindow(const std::wstring& windowName, int clientWidth, int clientHeight, bool vSync )
{
    ...
    auto pWindow = MakeRef<Window>(hWindow, windowName, clientWidth, clientHeight);
...
RefPtr<Window> Application::GetWindowByName(const std::wstring& windowName)
{
...
int Application::Run(RefPtr<Game> pGame)
{
...
Microsoft::WRL::ComPtr<ID3D12Device2> Application::GetD3D12Device() const
{
    return m_Device->GetD3D12Device();
}

RefPtr<Device> Application::GetDevice() const
{
    return m_Device;
}
