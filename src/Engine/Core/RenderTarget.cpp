...
void RenderTarget::ClearDepth(RefPtr<CommandList> commandList)
{
    commandList->ClearDepthStencilTexture(GetTexture(AttachmentPoint::DepthStencil), D3D12_CLEAR_FLAG_DEPTH);
}

void RenderTarget::Clear(RefPtr<CommandList> commandList, const XMFLOAT4& clearColor, bool clearDepth)
{
    commandList->ClearTexture(GetTexture(AttachmentPoint::Color0), &clearColor.x);
    if (clearDepth)
    {
        commandList->ClearDepthStencilTexture(GetTexture(AttachmentPoint::DepthStencil), D3D12_CLEAR_FLAG_DEPTH);
    }
